...
class db {
  ...
  static async sendNotifsActualPrice(bot, interval, priceBtcUsd, priceEthUsd) {
    try {
      ...
      const sendQueue = priceSubsRows.map(row => bot.telegram.sendMessage(row.dataValues.chatId,
        notifMsg));
  ...
  static async btcSendNotifsNewTx(bot, dataTx, priceBtcUsd) {
    ...
    const sendQueue = btcRows.map(row => bot.telegram.sendMessage(row.dataValues.chatId,
      notifMsg));

    await Promise.all(sendQueue);
  }

  static async ethSendNotifsNewTx(bot, eth, newBlock, priceEthUsd) {
    setTimeout(async () => {
