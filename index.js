...
const { template, xs } = require('./lib/template');
const { db, PriceWatchList } = require('./lib/db');
...
async function updatePriceSendNotif() {
  try {
    const [dataPrice, priceWatchersData] = await Promise.all([
      eth.stats.ethprice(),
      PriceWatchList.findAll(),
    ]);

    const btcPrice = dataPrice.result.ethusd / dataPrice.result.ethbtc;
    ...
    priceWatchersData.forEach((row) => {
      ...
      const isBtcAlert = coinName === 'bitcoin'
        && v.isPriceOutOfRange(price.btcUsd, priceLow, priceHigh);

      const isEthAlert = coinName === 'ethereum'
...
const blockchainErrMsg = 'Error. Blockchain api is not available or you entered '
...
blockchainMenu.on('text', async (ctx) => {
  ...
  try {
    if (v.isBtcAddress(message)) {
      const dataAddr = await btc.blockexplorer.getAddress(message, { limit: 5 });
      const replyMsg = template.btcAddress(dataAddr, message, price.btcUsd);

      await v.splitAndReply(ctx, replyMsg);
    } else if (v.isBtcTxOrBlock(message)) {
      try {
        const dataTx = await btc.blockexplorer.getTx(message);
        const replyMsg = template.btcTxItem(dataTx, price.btcUsd);

        await v.splitAndReply(ctx, replyMsg);
      } catch (noTx) {
        const dataBlock = await btc.blockexplorer.getBlock(message);
        const replyMsg = template.btcBlock(dataBlock);

        await v.splitAndReply(ctx, replyMsg);
      }
    } else if (v.isEthAddress(message)) {
      const [dataBalance, dataTxList] = await Promise.all([
        eth.account.balance(message),
        eth.account.txlist(message, 0, 'latest', 1, 5, 'desc'),
      ]);
      const replyMsg = template.ethAddress(dataBalance, dataTxList, message, price.ethUsd);

      await v.splitAndReply(ctx, replyMsg);
    } else if (v.isEthTx(message)) {
      const dataTx = await eth.proxy.eth_getTransactionByHash(message);
      const replyMsg = template.ethTxItem(dataTx, price.ethUsd);

      await v.splitAndReply(ctx, replyMsg);
    } else if (Number.isInteger(+message)) {
      const blockHex = `0x${(+message).toString(16)}`;
      const dataBlock = await eth.proxy.eth_getBlockByNumber(blockHex);
      const replyMsg = template.ethBlock(dataBlock);
...
watchAddrMenu.enter(async (ctx) => {
  ...
  try {
    const watchList = await db.getWatchListMsg(chatId);
...
watchAddrMenu.hears(/\/./, async (ctx) => {
  const chatId = ctx.chat.id;
  const message = ctx.message.text.trim().slice(1);

  const isBtcAddr = v.isBtcAddress(message);
  const isEthAddr = v.isEthAddress(message);
...
watchPriceMenu.enter(async (ctx) => {
  ...
  const watchPriceList = await db.getPriceWatchMsg(chat);
...
watchPriceMenu.on('text', async (ctx) => {
  const chat = ctx.chat.id;
  const message = ctx.message.text.trim();
  const validMsg = v.validateWatchPriceMsg(message);
...
subsPriceMenu.enter(async (ctx) => {
  const chat = ctx.chat.id;
  try {
    const getUserPriceSubs = await db.getUserPriceSubs(chat);
    if (getUserPriceSubs) {
...
wsBtc.addEventListener('message', async (msg) => {
  try {
    const dataTx = JSON.parse(msg.data);
